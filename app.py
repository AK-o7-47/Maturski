...
# -------------------------
# HELPER FUNKCIJE ZA AVIONE
# -------------------------

def get_avioni():
    """Dohvata sve avione iz baze podataka."""
    conn = get_db_connection()
    avioni = conn.execute("SELECT * FROM avioni ORDER BY naziv").fetchall()
    conn.close()
    
    # Konvertuj svaki Row u dict
    return [dict(a) for a in avioni]



def get_avion_by_id(avion_id):
    """Dohvata avion po ID-u iz baze podataka."""
    conn = get_db_connection()
    avion = conn.execute(
        "SELECT * FROM avioni WHERE id = ?", (avion_id,)
    ).fetchone()
    conn.close()
    ...
    # Konvertuj Row u dict da bi mogao da se serijalizuje u JSON
    return dict(avion)


def get_kapacitet_aviona(avion_id):
    """Dohvata kapacitet aviona po ID-u iz baze podataka."""
    avion = get_avion_by_id(avion_id)
    if avion:
        return avion["broj_sedista"]
    return 0
    ...
    if request.method == "POST":
        ...
        if postojeci:
            ...
            return render_template(
                "signup.html", greska="Korisnik sa tim emailom već postoji."
            )
...
def let_detalji(let_id):
    ...
    # Dohvata avion iz baze umesto iz liste
...
def api_reset_rezervacija(let_id):
    ...
    # Dohvata kapacitet iz baze umesto iz recnika
    kapacitet = get_kapacitet_aviona(let["tip_aviona"])
    ...
    return jsonify({
        "poruka": "Rezervacije su obrisane i broj slobodnih mesta je resetovan."
    })
...
def sve_rezervacije():
    ...
    query = """
        SELECT r.id, r.sediste, r.ime, r.prezime, r.pasos,
               ...
               l.id AS let_id, l.broj_leta, l.aviokompanija,
               l.datum_polaska, l.cena,
               ap.grad AS polaziste_grad, ap.iata_kod AS polaziste_kod,
               ao.grad AS odrediste_grad, ao.iata_kod AS odrediste_kod
        FROM rezervacije r
...
def izmeni_aerodrom(id):
    conn = get_db_connection()
    aerodrom = conn.execute(
        "SELECT * FROM aerodromi WHERE id = ?", (id,)
    ).fetchone()
...
@app.route("/avioni/dodaj", methods=["GET", "POST"])
@admin_required
def dodaj_avion():
    if request.method == "POST":
        avion_id = request.form["id"].strip().lower()
        naziv = request.form["naziv"].strip()
        broj_sedista = request.form["broj_sedista"].strip()

        conn = get_db_connection()

        # Provera da li vec postoji avion sa tim ID-om
        postojeci = conn.execute(
            "SELECT id FROM avioni WHERE id = ?", (avion_id,)
        ).fetchone()
        ...
        if postojeci:
            ...
            return render_template(
                "avion_forma.html",
                avion=None,
                greska="Avion sa tim ID-om već postoji."
            )

        conn.execute("""
            INSERT INTO avioni (id, naziv, broj_sedista)
            VALUES (?, ?, ?)
        """, (avion_id, naziv, broj_sedista))
        conn.commit()
        conn.close()

        return redirect(url_for("lista_aviona"))

    return render_template("avion_forma.html", avion=None)


@app.route("/avioni/izmeni/<string:avion_id>", methods=["GET", "POST"])
@admin_required
def izmeni_avion(avion_id):
    conn = get_db_connection()
    avion = conn.execute(
        "SELECT * FROM avioni WHERE id = ?", (avion_id,)
    ).fetchone()
    ...
    if avion is None:
        conn.close()
        return "Avion nije pronađen.", 404
    ...
    if request.method == "POST":
        ...
        conn.execute("""
            UPDATE avioni
            SET naziv = ?, broj_sedista = ?
            WHERE id = ?
        """, (naziv, broj_sedista, avion_id))
        conn.commit()
        conn.close()
    ...
    conn.close()
    return render_template("avion_forma.html", avion=avion)


@app.route("/avioni/obrisi/<string:avion_id>")
@admin_required
def obrisi_avion(avion_id):
    conn = get_db_connection()

    # Provera da li je avion u upotrebi
    u_upotrebi = conn.execute(
        "SELECT id FROM letovi WHERE tip_aviona = ?", (avion_id,)
    ).fetchone()
    ...
    if u_upotrebi:
        ...
        return render_template(
            "avioni.html",
            avioni=avioni,
            greska="Ne možete obrisati avion koji se koristi na letovima."
        )

    conn.execute("DELETE FROM avioni WHERE id = ?", (avion_id,))
    conn.commit()
    conn.close()
...
def lista_letova():
    ...
    # Dohvata avione iz baze umesto iz liste
    avioni = get_avioni()
...
def dodaj_let():
    ...
    avioni = get_avioni()
    return render_template(
        "let_forma.html", let=None, aerodromi=aerodromi, avioni=avioni
    )
...
def izmeni_let(id):
    ...
    if request.method == "POST":
        ...
        conn.execute("""
            UPDATE letovi
            SET broj_leta = ?, aviokompanija = ?, tip_aviona = ?,
                polaziste_id = ?, odrediste_id = ?,
                datum_polaska = ?, datum_dolaska = ?,
                cena = ?, broj_slobodnih_mesta = ?
            WHERE id = ?
        """, (
            broj_leta, aviokompanija, tip_aviona, polaziste_id, odrediste_id,
            datum_polaska, datum_dolaska, cena, broj_slobodnih_mesta, id
        ))
        conn.commit()
        conn.close()
    ...
    avioni = get_avioni()
    return render_template(
        "let_forma.html", let=let, aerodromi=aerodromi, avioni=avioni
    )
